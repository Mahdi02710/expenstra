...
class LocalDatabaseService {
  ...
  Future<T> _withDb<T>(Future<T> Function(Database db) action) {
  ...
  Future<void> deleteTransaction(String id, {bool synced = false}) async {
    await _withDb((db) async {
      ...
      if (!synced) {
        await _addToSyncQueueWithDb(db, _transactionsTable, id, 'delete', null);
      }
    });
  }
  ...
  Future<void> deleteWallet(String id, {bool synced = false}) async {
    await _withDb((db) async {
      ...
      if (!synced) {
        await _addToSyncQueueWithDb(db, _walletsTable, id, 'delete', null);
      }
    });
  }
  ...
  Future<void> deleteBudget(String id, {bool synced = false}) async {
    await _withDb((db) async {
      ...
      if (!synced) {
        await _addToSyncQueueWithDb(db, _budgetsTable, id, 'delete', null);
      }
    });
  }
  ...
  Future<void> _addToSyncQueueWithDb(
    Database db,
    String tableName,
    String recordId,
    String operation,
